...
interface GestureProcessor {
    ...
    /**
     * Triggered whe na tap occurred. A tap occurs when a pointer touches and lets go of the same area of the screen
     * without moving outside the tap zone. The tap zone is a rectangular area around the initial tap position.
     *
    ...
    /**
     * Triggered when a user "flings" the screen by dragging and lifting their pointer. Reports last velocity in pixels
     * per second.
...
class GestureProcessorBuilder {
     ...
     */
    fun build() = object : GestureProcessor {
        ...
        override fun tap(screenX: Float, screenY: Float, count: Int, pointer: Pointer): Boolean {
            var handled = false
        ...
        override fun pinch(initialPos1: Vec2f, initialPos2: Vec2f, pos1: Vec2f, pos2: Vec2f): Boolean {
            var handled = false
            pinch.fastForEach { if (it.invoke(initialPos1, initialPos2, pos1, pos2)) handled = true }
            return handled
        }
