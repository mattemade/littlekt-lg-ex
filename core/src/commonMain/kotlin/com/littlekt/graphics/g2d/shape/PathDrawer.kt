 ...
 * @date 7/19/2022
 */
internal class PathDrawer(batchManager: BatchManager, private val lineDrawer: LineDrawer) : Drawer(batchManager) {
    ...
    fun path(
        ...
        for (i in start + 2 until end step 2) {
            if (!isFuzzyEqual(userPath[i - 2], userPath[i], 0.001f) || !isFuzzyEqual(userPath[i - 1],
                    userPath[i + 1],
                    0.001f)
            ) {
                path += userPath[i]
    ...
    fun drawPathWithJoin(path: FloatArray, size: Int, thickness: Float, open: Boolean, pointyJoin: Boolean) {
        ...
        for (i in 2 until size - 2 step 2) {
            ...
            if (i == 2) {
                if (open) {
                    prepareFlatEndpoint(path[2],
                        path[3], path[0], path[1], d, e, halfThickness)
                    vert1(e)
                    vert2(d)
                } else {
