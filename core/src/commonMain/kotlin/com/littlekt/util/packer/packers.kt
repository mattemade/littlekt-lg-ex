...
class MaxRectsPacker(val options: PackingOptions) : Packer {
    ...
    val rects: List<BinRect> get() = bins.flatMap { it.rects }
    ...
    override fun load(bins: List<Bin>) {
        ...
        bins.forEach { bin ->
            if (bin.maxWidth > width || bin.maxHeight > height) {
                ...
                newBin._freeRects = bin.freeRects.map { BinRect(it.x, it.y, it.width, it.height) }.toMutableList()
                newBin._rects = bin.rects.map { BinRect(it.x, it.y, it.width, it.height) }.toMutableList()
