...
import com.littlekt.util.datastructure.internal.arraycopy
import com.littlekt.util.datastructure.internal.contentEquals
import com.littlekt.util.datastructure.internal.contentHashCode
...
class ShortArrayList(capacity: Int = 7, var shouldGrow: Boolean = true) : Collection<Short> {
    ...
    constructor(other: ShortArrayList) : this() {
        add(other)
    }

    constructor(vararg other: Short) : this() {
        add(other)
    }
    ...
    operator fun plusAssign(value: Short) = add(value)
    operator fun plusAssign(value: ShortArray) = add(value)
    operator fun plusAssign(value: ShortArrayList) = add(value)
    operator fun plusAssign(value: Iterable<Short>) = add(value)
    ...
    fun add(values: ShortArrayList) = add(values.data, 0, values.size)
    ...
    fun setAt(index: Int, value: Short): Short = value.also { set(index, value) }
    ...
    override fun iterator(): Iterator<Short> = listIterator(0)
    ...
    fun insertAt(index: Int, value: Short) = this.apply {
        ensure(1)
    ...
    fun insertAt(index: Int, value: ShortArray, start: Int = 0, end: Int = value.size) = this.apply {
        val count = end - start
        ensure(count)
    ...
    fun removeAt(index: Int): Short = removeAt(index, 1)
    ...
    fun indexOf(element: Short): Int = indexOf(element, 0, size)
    fun lastIndexOf(element: Short): Int = lastIndexOf(element, 0, size)

    fun listIterator(): ListIterator<Short> = listIterator(0)
    fun listIterator(index: Int): ListIterator<Short> = data.take(length).listIterator(index)
    fun subList(fromIndex: Int, toIndex: Int): List<Short> = data.asList().subList(fromIndex, toIndex)

    // Data
    override fun hashCode(): Int = data.contentHashCode(0, size)
    override fun equals(other: Any?): Boolean {
        if (other is ShortArrayList) return size == other.size && data.contentEquals(other.data, 0, size)
...
fun shortArrayListOf(vararg values: Short) = ShortArrayList(*values)
fun ShortArray.toShortArrayList() = ShortArrayList(*this)
