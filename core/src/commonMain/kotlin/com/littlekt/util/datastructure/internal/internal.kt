...
internal fun <T> Array<T>.fill(value: T) {
    for (n in 0 until this.size) this[n] = value
}

internal fun IntArray.fill(value: Int) {
    for (n in 0 until this.size) this[n] = value
}
...
internal fun DoubleArray.contentHashCode(src: Int, dst: Int): Int =
    hashCoder(dst - src) { this[src + it].toInt() } // Do not want to use Long (.toRawBits) to prevent boxing on JS
...
internal fun DoubleArray.contentEquals(that: DoubleArray, src: Int, dst: Int): Boolean =
    equaler(dst - src) { this[src + it] == that[src + it] } // Do not want to use Long (.toRawBits) to prevent boxing on JS
