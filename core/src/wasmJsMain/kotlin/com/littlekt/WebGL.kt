...
class WebGL(val gl: WebGL2RenderingContext, val platform: Context.Platform, private val engineStats: EngineStats) : GL {
    ...
    override fun getProgramiv(glShaderProgram: GlShaderProgram, pname: Int, params: IntBuffer) {
        ...
        if (pname == GetProgram.DELETE_STATUS.glFlag || pname == GetProgram.LINK_STATUS.glFlag || pname == GetProgram.VALIDATE_STATUS.glFlag) {
            val result: Boolean = (gl.getProgramParameter(glShaderProgram.delegate, pname) as JsBoolean).toBoolean()
            params.put(if (result) GL.TRUE else GL.FALSE)
        } else {
            params.put((gl.getProgramParameter(glShaderProgram.delegate, pname) as JsNumber).toInt())
        }
        params.flip()
    }
    ...
    override fun getString(pname: Int): String? {
        engineStats.calls++
        return gl.getParameter(pname)?.toString()
    }
    ...
    override fun getIntegerv(pname: Int, data: IntBuffer) {
        ...
        when (pname) {
            ...
            -> {
                data[0] = gl.getParameter(pname).toString().toInt()
                data.flip()
            }
    ...
    override fun uniformMatrix3fv(uniformLocation: UniformLocation, transpose: Boolean, data: Array<Float>) {
        engineStats.calls++
        gl.uniformMatrix3fv(uniformLocation.delegate, transpose, data.toFloatArray().toFloat32Array())
    }
    ...
    override fun uniformMatrix4fv(uniformLocation: UniformLocation, transpose: Boolean, data: Array<Float>) {
        engineStats.calls++
        gl.uniformMatrix4fv(uniformLocation.delegate, transpose, data.toFloatArray().toFloat32Array())
    }
    ...
    override fun uniform1fv(uniformLocation: UniformLocation, floats: FloatArray) {
        engineStats.calls++
        gl.uniform1fv(uniformLocation.delegate, floats.toFloat32Array())
    }

    override fun uniform2fv(uniformLocation: UniformLocation, floats: FloatArray) {
        engineStats.calls++
        gl.uniform2fv(uniformLocation.delegate, floats.toFloat32Array())
    }

    override fun uniform3fv(uniformLocation: UniformLocation, floats: FloatArray) {
        engineStats.calls++
        gl.uniform3fv(uniformLocation.delegate, floats.toFloat32Array())
    }

    override fun uniform4fv(uniformLocation: UniformLocation, floats: FloatArray) {
        engineStats.calls++
        gl.uniform4fv(uniformLocation.delegate, floats.toFloat32Array())
    }
    ...
    override fun texImage3D(
        ...
        gl.texImage3D(
            ...
            source.toArray().toUint8Array() // convert it to a uint8array or else webgl fails to render
        )
    }
