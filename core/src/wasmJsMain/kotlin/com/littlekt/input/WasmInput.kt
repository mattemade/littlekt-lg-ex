 ...
 * @date 11/22/2021
 */
class WasmInput(val canvas: HTMLCanvasElement) : Input {
                ...
                for (gamepadId in 0 until jsGamepads.length) {
                    ...
                    gamepad.apply {
                        ...
                        for (n in 0 until controller.axes.length) {
                            this.rawAxes[n] = controller.axes[n]!!.unsafeCast<JsNumber>().toDouble().toFloat()
                        }
...
private external interface JsGamepadButton : JsAny {
    val value: Float
    val pressed: Boolean
}

private external interface JsGamePad : JsAny {
