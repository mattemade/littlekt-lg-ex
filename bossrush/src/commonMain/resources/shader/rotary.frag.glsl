...
uniform float u_heightToWidthRatio;
...
void main() {
    float skewdOffset = (1.0 - u_heightToWidthRatio)/2.0;
    vec2 coordInSquare = vec2(v_texCoords.x, skewdOffset + v_texCoords.y * u_heightToWidthRatio);
    ...
    coordInSquare += halfTexture;
    coordInSquare.y = (coordInSquare.y - skewdOffset) / u_heightToWidthRatio;
